...
Engine::Engine() : m_window(WindowConfig{}), m_camera(1280, 720)
{
    ...
    m_spriteShaderHandle = m_assets.load<ShaderProgram>("sprite", "assets/shaders/sprite.vert",
...
void Engine::update(float dt)
{
    ...
    if (m_input.isActionHeld("cam_left"))
        m_camera.position.x -= speed * dt;
    if (m_input.isActionHeld("cam_right"))
        m_camera.position.x += speed * dt;
    if (m_input.isActionHeld("cam_up"))
        m_camera.position.y += speed * dt;
    if (m_input.isActionHeld("cam_down"))
        m_camera.position.y -= speed * dt;
