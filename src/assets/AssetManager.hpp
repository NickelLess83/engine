...
public:
    ...
    Handle<T> load(const std::string& key, Args&&... args)
    {
    ...
    template<typename T>
    Handle<T> get(const std::string& key) const
    {
    ...
    void unloadAll()
    {
        m_pools.erase(std::type_index(typeid(T)));
    }

private:
    template<typename T>
    AssetPool<T>& getOrCreate()
    {
    ...
    template<typename T>
    const AssetPool<T>& getPool() const
    {
        auto it = m_pools.find(std::type_index(typeid(T)));
