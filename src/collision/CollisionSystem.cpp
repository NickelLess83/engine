...
static bool testAABBvsAABB(AABB2 a, AABB2 b, glm::vec2 &normal, float &depth)
{
    ...
    if (dx1 <= 0.0f || dx2 <= 0.0f || dy1 <= 0.0f || dy2 <= 0.0f)
        return false;
...
static bool testCirclevsCircle(glm::vec2 ca, float ra, glm::vec2 cb, float rb, glm::vec2 &normal,
                               float &depth)
{
    glm::vec2 d = cb - ca;
...
static bool testAABBvsCircle(AABB2 box, glm::vec2 center, float radius, glm::vec2 &normal,
                             float &depth)
{
...
// Broadphase helpers
// --------------------------------------------------------------------------
static uint64_t cellKey(int cx, int cy) { return (uint64_t)(uint32_t)cx << 32 | (uint32_t)cy; }
...
static void registerInGrid(glm::vec2 bpMin, glm::vec2 bpMax, Entity entity, float cellSize,
    ...
    for (auto &[ck, entities] : grid) {
        for (size_t i = 0; i < entities.size(); ++i) {
            for (size_t j = i + 1; j < entities.size(); ++j) {
                ...
                if (!checked.insert(pairKey(ea, eb)).second)
                    continue;
                ...
                if (reg.has<CollisionLayer>(ea) && reg.has<CollisionLayer>(eb)) {
                    ...
                    if (!(la.mask & lb.layer) && !(lb.mask & la.layer))
                        continue;
                }
                ...
                if (ca.shape == Collider::Shape::AABB && cb.shape == Collider::Shape::AABB) {
                    hit = testAABBvsAABB({posA - ca.halfExtents, posA + ca.halfExtents},
                                         {posB - cb.halfExtents, posB + cb.halfExtents}, normal,
                                         depth);
                } else if (ca.shape == Collider::Shape::Circle &&
                           cb.shape == Collider::Shape::Circle) {
                    ...
                    if (ca.shape == Collider::Shape::AABB) {
                        hit = testAABBvsCircle({posA - ca.halfExtents, posA + ca.halfExtents}, posB,
                                               cb.radius, normal, depth);
                    } else {
                        hit = testAABBvsCircle({posB - cb.halfExtents, posB + cb.halfExtents}, posA,
                                               ca.radius, normal, depth);
